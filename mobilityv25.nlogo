...
to go
  ...
  if year mod 100 = 0 and evi-counter = 1
  [ export-world (word "run_" run-counter "_eco_social_cc_" year ".csv")
    export-view (word "run_" run-counter "_eco_social_cc_" year ".png")]
...
to agent-commands
  ...
  [ if experiment-type = "null"
    ...
    discard
    demographics
    if experiment-type = "social-only" or experiment-type = "eco-social" or experiment-type = "eco-social-climatechange"
    [ social ]
  ]
end
...
to move-decision
  ...
  if experiment-type = "eco-only" or experiment-type = "eco-social" or experiment-type = "eco-social-climatechange"
  ...
  [ if move? = "no"
    ...
    if experiment-type = "social-only"
    [ if move? = "no"
      [ if random 100 < 25 [ set move? "yes" ] ] ] ]
...
to move
  ...
  [ let maxmovepatches other patches in-radius maxmove with [ envirotype != "marine" ]
    ...
    [ let tmptarget nobody
      ask focus-camp [ set tmptarget one-of patches in-radius maxmove with [ envirotype != "marine" ] ]
  ...
  if experiment-type = "social-only"
  [ let patch-targets patches in-radius maxmove with [ envirotype != "marine" ]
    ifelse focus-camp = nobody
    [ set target one-of patch-targets ]
    [ let tmptarget nobody
      ifelse focus-interaction = "positive"
      [ ask focus-camp [ set tmptarget one-of patches in-radius maxmove with [ envirotype != "marine" ]
        set target tmptarget ] ]
      [ set target max-one-of patches in-radius maxmove with [ envirotype != "marine" ] [ distance focus-camp ] ] ] ]
...
to forage
  ...
  if new-plant-resources < 0 [ set new-plant-resources 0 ]
  if new-shellfish-resources < 0 [ set new-plant-resources 0 ]
...
to demographics
  ...
  [ set tmpfertility ( fertility-rate / 10000 ) * population-size
    set tmpmortality ( mortality-rate / 100 ) * population-size ]
  ...
  [ if any? other camps in-radius 2
    [ create-affiliations-with other camps in-radius 2
      [ set affinity one-of [ 0.3 -0.3 ]
        ...
        set memory-end1 "none"
        set memory-end2 "none" ] ] ]
end

to interact
  let endone-choice "none"
  let endtwo-choice "none"
  if memory-end1 = "none" and memory-end2 = "none"
  [ set endone-choice one-of [ "cooperate" "defect" ]
    set endtwo-choice one-of [ "cooperate" "defect" ] ]
  if memory-end1 = "cooperate" and memory-end2 = "cooperate"
  [ ifelse random 100 < 80 [ set endone-choice [ "cooperate" ] ] [ set endone-choice [ "defect" ] ]
    ifelse random 100 < 80 [ set endtwo-choice [ "cooperate" ] ] [ set endone-choice [ "defect" ] ] ]
  if memory-end1 = "cooperate" and memory-end2 = "defect"
  [ ifelse random 100 < 80 [ set endone-choice [ "cooperate" ] ] [ set endone-choice [ "defect" ] ]
    ifelse random 100 < 80 [ set endtwo-choice [ "defect" ] ] [ set endone-choice [ "cooperate" ] ] ]
  if memory-end1 = "defect" and memory-end2 = "cooperate"
  [ ifelse random 100 < 80 [ set endone-choice [ "defect" ] ] [ set endone-choice [ "cooperate" ] ]
    ifelse random 100 < 80 [ set endtwo-choice [ "cooperate" ] ] [ set endone-choice [ "defect" ] ] ]
  if memory-end1 = "defect" and memory-end2 = "defect"
  [ ifelse random 100 < 80 [ set endone-choice [ "defect" ] ] [ set endone-choice [ "cooperate" ] ]
    ifelse random 100 < 80 [ set endtwo-choice [ "defect" ] ] [ set endone-choice [ "cooperate" ] ] ]

  if endone-choice = "cooperate" and endtwo-choice = "cooperate"
  [ ifelse affinity < 0.98 [ set affinity affinity + 0.03 ] [ set affinity 1 ] ]
  if endone-choice = "cooperate" or endtwo-choice = "defect"
  [ ifelse affinity > -1 [ set affinity affinity - 0.01 ] [ set affinity -1 ] ]
  if endone-choice = "defect" or endtwo-choice = "cooperate"
  [ ifelse affinity > -1 [ set affinity affinity - 0.01 ] [ set affinity -1 ] ]
  if endone-choice = "defect" and endtwo-choice = "defect"
  [ ifelse affinity > -0.98 [ set affinity affinity - 0.03 ] [ set affinity -1 ] ]
...
to update-resources
  let cc 0
  if experiment-type = "eco-social-climatechange"
  [ set cc ( sealevelmod / 300 ) ]

  if evi-counter = 81
  [ let evi-scaler max [ evi-current ] of patches
    ask patches with [ biome-code = 1 ] [ set plant-resources ( ( item 3 forest_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 2 ] [ set plant-resources ( ( item 3 fynbos_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 3 ] [ set plant-resources ( ( item 3 grassland_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 4 ] [ set plant-resources ( ( item 3 nama_karoo_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 5 ] [ set plant-resources ( ( item 3 savanna_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 6 ] [ set plant-resources ( ( item 3 succulent_karoo_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 7 ] [ set plant-resources ( ( item 3 thicket_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 8 ] [ set plant-resources ( ( item 3 desert_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    update-maxima ]

  if evi-counter = 173
  [ let evi-scaler max [ evi-current ] of patches
    ask patches with [ biome-code = 1 ] [ set plant-resources ( ( item 0 forest_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 2 ] [ set plant-resources ( ( item 0 fynbos_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 3 ] [ set plant-resources ( ( item 0 grassland_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 4 ] [ set plant-resources ( ( item 0 nama_karoo_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 5 ] [ set plant-resources ( ( item 0 savanna_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 6 ] [ set plant-resources ( ( item 0 succulent_karoo_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 7 ] [ set plant-resources ( ( item 0 thicket_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 8 ] [ set plant-resources ( ( item 0 desert_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    update-maxima ]

  if evi-counter = 265
  [ let evi-scaler max [ evi-current ] of patches
    ask patches with [ biome-code = 1 ] [ set plant-resources ( ( item 1 forest_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ]  ]
    ask patches with [ biome-code = 2 ] [ set plant-resources ( ( item 1 fynbos_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ]  ]
    ask patches with [ biome-code = 3 ] [ set plant-resources ( ( item 1 grassland_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 4 ] [ set plant-resources ( ( item 1 nama_karoo_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 5 ] [ set plant-resources ( ( item 1 savanna_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 6 ] [ set plant-resources ( ( item 1 succulent_karoo_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 7 ] [ set plant-resources ( ( item 1 thicket_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 8 ] [ set plant-resources ( ( item 1 desert_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    update-maxima ]

  if evi-counter = 356 or ticks = 0
  [ let evi-scaler max [ evi-current ] of patches
    ask patches with [ biome-code = 1 ] [ set plant-resources ( ( item 2 forest_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 2 ] [ set plant-resources ( ( item 2 fynbos_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 3 ] [ set plant-resources ( ( item 2 grassland_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 4 ] [ set plant-resources ( ( item 2 nama_karoo_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 5 ] [ set plant-resources ( ( item 2 savanna_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 6 ] [ set plant-resources ( ( item 2 succulent_karoo_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 7 ] [ set plant-resources ( ( item 2 thicket_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    ask patches with [ biome-code = 8 ] [ set plant-resources ( ( item 2 desert_resources ) * ( evi-current / evi-scaler ) ) set plant-resources plant-resources + ( plant-resources * cc ) if plant-resources < 0 [ set plant-resources 0 ] ]
    update-maxima ]
...
experiment-type
experiment-type
"null" "eco-only" "social-only" "eco-social" "eco-social-climatechange"
0
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="run-counter">
      <value value="1"/>
      <value value="2"/>
      <value value="3"/>
      <value value="4"/>
